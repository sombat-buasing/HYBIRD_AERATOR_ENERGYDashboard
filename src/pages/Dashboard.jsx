...
function Dashboard() {
  ...
  const loadData = async () => {
    try {
      ...
      if (!selectedDevice && data.length > 0) {
        ...
        loadMonthlyReport(data[0].device_id);
      }
  ...
  const loadDeviceConfig = async () => {
    try {
      ...
      if (devices.length > 0 && !selectedDevice) {
        console.log("SET DEVICE", devices[0].device_id);
  ...
  const loadMonthlyReport = async (deviceId) => {
    // console.log("loadMonthlyReport:", deviceId);
  ...
  const monthCost = monthly.length > 0 ? Number(monthly[0].energy_kwh) * 5 : 0;
      ...
      >
        ...
        >
          <CardContent sx={{ textAlign: "center" }}>
            ...
            <Typography variant="h3">
              {monthly.length > 0
                ? Number(monthly[0].energy_kwh).toFixed(3)
                : "0.000"}
            </Typography>
          ...
          <FormControl fullWidth>
            ...
            <Select
              ...
              onChange={(e) => {
                ...
                loadTopPeakEvents(deviceId);
                loadMonthlyReport(deviceId);
              }}
            >
              {devices.map((d) => (
            ...
            <Table>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell align="right">Cost (THB)</TableCell>
                </TableRow>
              </TableHead>
              ...
              <TableBody>
                {monthlyReport.map((r) => {
                  const cost =
                    Number(r.energy_kwh) * Number(config?.unit_price || 0);
                  ...
                  return (
                    <TableRow key={r.report_month}>
                      ...
                      <TableCell align="right">
                        {Number(r.energy_kwh).toFixed(2)}
                      </TableCell>

                      <TableCell align="right">
                        {Number(r.peak_kw).toFixed(3)}
                      </TableCell>
